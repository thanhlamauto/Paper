\section{ReAlign Method Details}
\label{app:realign_algorithm_compute}

This appendix keeps the main paper compact while making the implementation easy to audit. We organize the details into four groups: ReAlign method details, experimental protocols, additional ablations, and representation diagnostics. Cross-references from the main text point to the specific subsection needed for each result.

\subsection{Training Algorithm and Compute}

ReAlign adds a lightweight training-time branch to the diffusion transformer. The backbone forward pass exposes source and target activations, the shared predictor maps the source state into target-layer coordinates, and the auxiliary loss combines transition prediction, path consistency, and weak-layer diversity. A small fraction of the minibatch can feed the predicted target state into the remaining blocks inside the ordinary flow-matching loss.

\begin{algorithm}[!htbp]
\caption{\textbf{ReAlign training.} One flow-matching loss with a shared cross-depth transition predictor.}
\label{alg:realign_training}
\small
\begin{algorithmic}[1]
\Require backbone $F_\zeta$ with block ranges $F_{\zeta,i:j}$; predictor $g_\psi$; EMA predictor $\bar g_\psi$; pair law $q(a,b,r)$; FM-path rate $\rho$; weights $\lambda_{\mathrm{pred}},\lambda_{\mathrm{cons}},\lambda_{\mathrm{div}}$.
\Repeat
    ...
    \end{aligned}
    \]
    \State Compute the ReAlign regularizer:
    \[
    ...
    \]
    \State Update $\zeta,\psi$ with $\mathcal{L}_{\mathrm{FM}}+\mathcal{L}_{\mathrm{ReAlign}}$.
...
\begin{table}[!htbp]
    ...
    \begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}lccccccc@{}}
        ...
        ReAlign & 5--10\% & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} & $1.00\times$ \\
        ReAlign interface Euler & 5--10\% & -- & -- & -- & -- & -- & \textbf{[TBD]} \\
        \bottomrule
    \end{tabular*}
\end{table}
...
\subsection{ReAlign Hyperparameters}
...
\begin{table}[!htbp]
    \centering
    \caption{\textbf{ReAlign hyperparameter setup.} Placeholder table for the architecture and loss settings used in main results and ablations.}
    \label{tab:appendix_realign_hparams}
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{lcccccccccc}
        \toprule
        Experiment & Predictor & Pair sampling & Time cond. & $\lambda_{\mathrm{pred}}$ & $\lambda_{\mathrm{cons}}$ & $\lambda_{\mathrm{div}}$ & $\rho_{\mathrm{path}}$ & EMA decay & Target detach & Extra cost \\
        \midrule
        ...
        FM-path ablation & \textbf{[TBD]} & \textbf{[TBD]} & \checkmark & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} & varies & \textbf{[TBD]} & \checkmark & \textbf{[TBD]} \\
...
We use the same SiT-style transformer stack as the internal-alignment baselines~\citep{ma2024sit,haghighi2026layersync}. \Cref{fig:appendix_sit_realign_architecture} shows the reference stack and where ReAlign attaches its shared transition predictor.
...
\begin{figure}[!htbp]
    ...
    \begin{subfigure}[t]{0.55\linewidth}
        ...
        \caption{ReAlign attaches a shared predictor to sampled source--target layers.}
    \end{subfigure}
    \caption{\textbf{SiT stack and ReAlign attachment.} The backbone follows the same SiT block stack used by LayerSync~\citep{haghighi2026layersync}. ReAlign keeps the stack unchanged and adds a training-time transition predictor between sampled depths.}
...
\begin{table}[!htbp]
    \centering
    \caption{\textbf{HumanML3D setup.} Motion-generation configuration used by the baseline, LayerSync, and ReAlign variants.}
    ...
    \begin{tabular}{lccccc}
        ...
        MDM + ReAlign & 6 & 600K & (3,6) & Standard MDM & FID, R-Precision \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[!htbp]
    ...
    \begin{tabular*}{0.98\linewidth}{@{\extracolsep{\fill}}p{0.22\linewidth}p{0.29\linewidth}p{0.31\linewidth}p{0.11\linewidth}@{}}
        ...
        \Cref{tab:humanml3d_results} & FID, R-Precision & Use the standard HumanML3D evaluator, split, text encoder, and motion representation for baseline, LayerSync, and ReAlign. & \citep{guo2022generating,tevet2022mdm} \\
...
The main ablation tables isolate the components needed for ReAlign. This appendix reports the broader sweeps that are useful for interpretation but too detailed for the main paper.
...
These controls tune the final recipe without changing the main ReAlign mechanism. We report them in the appendix because they select practical hyperparameters and keep the main paper focused.

\begin{table}[!htbp]
    \centering
    \caption{\textbf{Loss-weight and schedule controls.} Placeholder for tuning ReAlign component weights, maximum layer gap, and pair/timestep schedule.}
...
Scaling runs test whether the same ReAlign recipe transfers across \texttt{B/2}, \texttt{L/2}, and \texttt{XL/2} backbones. The comparison uses matched training settings within each size.
...
\begin{figure}[!htbp]
    ...
    \begin{minipage}[c][0.22\textheight][c]{0.88\linewidth}
        ...
        Plot baseline vs. ReAlign across training iterations. Suggested axes: FID, validation loss, or representation metric.
    \end{minipage}}
    \caption{\textbf{Training dynamics across model scales.} Placeholder for convergence curves comparing baseline and ReAlign-augmented backbones.}
    \label{fig:appendix_training_curves}
\end{figure}

\begin{table}[!htbp]
    \centering
    \caption{\textbf{ReAlign across model sizes.} Placeholder table for comparing baseline and ReAlign variants under matched training settings.}
    ...
    \begin{tabular}{lcccc}
        ...
        \texttt{B/2} & \textbf{[TBD]} & Baseline + ReAlign & \textbf{[TBD]} & \textbf{[TBD]} \\
        \texttt{L/2} & \textbf{[TBD]} & Baseline & \textbf{[TBD]} & \textbf{[TBD]} \\
        \texttt{L/2} & \textbf{[TBD]} & Baseline + ReAlign & \textbf{[TBD]} & \textbf{[TBD]} \\
        \texttt{XL/2} & \textbf{[TBD]} & Baseline & \textbf{[TBD]} & \textbf{[TBD]} \\
        \texttt{XL/2} & \textbf{[TBD]} & Baseline + ReAlign & \textbf{[TBD]} & \textbf{[TBD]} \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{itemize}[leftmargin=*]
    ...
    \item ReAlign with a shared conditional transition predictor.
\end{itemize}
...
\begin{figure}[!htbp]
    ...
    \begin{minipage}[c][0.22\textheight][c]{0.9\linewidth}
        ...
        Axes: layer index and layer distance; curves for baseline, LayerSync variants, and ReAlign.
    \end{minipage}}
...
\begin{table}[!htbp]
    \centering
    \caption{\textbf{Near vs. far layer alignment.} Placeholder for evaluating whether projection heads and ReAlign become more important as the layer gap increases.}
    ...
    \begin{tabular}{lccccc}
        ...
        ReAlign & Near & Shared transition & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} \\
        ReAlign & Far & Shared transition & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} \\
        \bottomrule
    \end{tabular}}
\end{table}
...
\begin{table}[!htbp]
    \centering
    \caption{\textbf{Held-out transition generalization.} Placeholder for testing whether ReAlign generalizes across unseen layer pairs better than pair-specific projectors.}
    ...
    \begin{tabular}{lccc}
        ...
        ReAlign shared transition & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} \\
        \bottomrule
    \end{tabular}}
\end{table}
...
Qualitative examples use matched noise, sampler, guidance, and checkpoints across baseline, Dispersive, LayerSync-style alignment, and ReAlign. They supplement the quantitative metrics by showing visible failure modes and sample-level differences under the same random seed.
...
\begin{figure}[!htbp]
    ...
    \begin{minipage}[c][0.24\textheight][c]{0.9\linewidth}
        ...
        Rows: baseline, Dispersive, LayerSync-style alignment, ReAlign.\\
