...
\Repeat
    ...
    \operatorname{cos}_{\mathrm{tok}}^2(A_i^t,A_j^t),
...
\end{equation}
The diversity term uses the same cached activations and squares the tokenwise cosine before averaging:
\begin{equation}
\operatorname{cos}_{\mathrm{tok}}^2(U,V)
=
\frac{1}{N}\sum_{n=1}^{N}
\left(
\frac{\langle U_n,V_n\rangle}
{\|U_n\|_2\|V_n\|_2+\varepsilon}
\right)^2.
\label{eq:appendix_token_cosine_squared}
\end{equation}
Thus $\mathcal{L}_{\mathrm{div}}$ is computed directly from the hidden-state cache $\mathcal{H}$ and does not require an additional backbone forward pass.
...
\begin{table}[!htbp]
    ...
    \begin{tabular*}{0.98\linewidth}{@{\extracolsep{\fill}}p{0.22\linewidth}p{0.29\linewidth}p{0.31\linewidth}p{0.11\linewidth}@{}}
        ...
        \Cref{tab:realign_ablation} & FID-10K & Use the same TPU budget, batch size, iterations, sampler, VAE, and reference statistics; only objective terms change. & \citep{heusel2017gans} \\
