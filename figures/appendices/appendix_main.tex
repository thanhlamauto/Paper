...
\paragraph{Tokenwise distances.}
For two token activations $U,V\in\mathbb{R}^{N\times C}$, LARA computes cosine similarity independently at each token and then averages over tokens:
\begin{equation}
\operatorname{cos}_{\mathrm{tok}}(U,V)
=
\frac{1}{N}\sum_{n=1}^{N}
\frac{\langle U_n,V_n\rangle}
{\|U_n\|_2\|V_n\|_2+\varepsilon}.
\label{eq:appendix_token_cosine}
\end{equation}
The transition distance used in \Cref{alg:realign_training} is the corresponding tokenwise cosine distance,
\begin{equation}
d(U,V)
=
\frac{1}{N}\sum_{n=1}^{N}
