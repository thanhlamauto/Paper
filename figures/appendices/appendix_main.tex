...
\caption{\textbf{LARA training.} One flow-matching loss with a shared cross-depth transition predictor.}
\label{alg:realign_training}
\small
\begin{algorithmic}[1]
\Require backbone $F_\zeta$; predictor $g_\psi$; EMA predictor $g_{\mathrm{ema}}$; pair law $q(a,b,r)$; weights $\lambda_{\mathrm{pred}},\lambda_{\mathrm{cons}},\lambda_{\mathrm{div}}$.
\Repeat
    ...
    \State Sample $(a,b,r)\sim q(a,b,r)$ with $a<b<r$.
    \State Run the full backbone once:
    \[
    (\hat v,A_a^t,A_b^t,A_r^t,\{A_i^t\})\leftarrow F_\zeta(x_t,t,y).
    \]
    \State Compute the flow-matching loss:
    \[
    \mathcal{L}_{\mathrm{FM}}
    =
    \mathbb{E}_{\mathcal{B}}\|\hat v-u_t\|_2^2.
    \]
    \State Compute transition and consistency predictions:
    \[
    \begin{aligned}
    \hat A_{a\to b}^t &= g_\psi(A_a^t;a,b,t),\\
    \hat A_{a\to r}^t &= g_\psi(A_a^t;a,r,t),\\
    \tilde A_{a\to r}^t
    &= g_{\mathrm{ema}}\!\left(g_{\mathrm{ema}}(\mathrm{sg}(A_a^t);a,b,t);b,r,t\right).
    \end{aligned}
    \]
    \State Compute the LARA regularizer:
    \[
    \mathcal{L}_{\mathrm{LARA}}
    =
    \lambda_{\mathrm{pred}}d(\hat A_{a\to b}^t,\mathrm{sg}(A_b^t))
    +\lambda_{\mathrm{cons}}d(\hat A_{a\to r}^t,\mathrm{sg}(\tilde A_{a\to r}^t))
    +\lambda_{\mathrm{div}}\mathbb{E}_{i<j}\operatorname{cos}^2(A_i^t,A_j^t).
    \]
...
We use a node of 4 GH200 GPUs and a batch size of 256. The details of hyperparameters and sampler are provided in \Cref{tab:appendix_imagenet_protocol,tab:appendix_ablation_protocol}.
...
We use the same SiT-style transformer stack as the internal-alignment baselines~\citep{ma2024sit,haghighi2026layersync}. \Cref{fig:appendix_sit_realign_architecture} shows the reference stack and where LARA attaches its shared transition predictor.
...
\begin{figure}[!htbp]
    ...
    \begin{subfigure}[t]{0.55\linewidth}
        \centering
        \includegraphics[height=0.34\textheight,keepaspectratio]{figures/realign.png}
        \caption{LARA attaches a shared predictor to sampled source--target layers.}
    \end{subfigure}
    \caption{\textbf{SiT stack and LARA attachment.} The backbone follows the same SiT block stack used by LayerSync~\citep{haghighi2026layersync}. LARA keeps the stack unchanged and adds a training-time transition predictor between sampled depths.}
    \label{fig:appendix_sit_realign_architecture}
\end{figure}
...
We use the exact setup as MDM~\citep{tevet2022mdm}. We train up to 600K iterations using a H100 GPU and align block 3 with block 6.
