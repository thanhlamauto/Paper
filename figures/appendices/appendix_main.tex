...
\small
\begin{algorithmic}[1]
\Require backbone $F_\zeta$ with block ranges $F_{\zeta,i:j}$; predictor $g_\psi$; EMA predictor $\bar g_\psi$; pair law $q(a,b,r)$; FM-path rate $\rho$; weights $\lambda_{\mathrm{align}},\beta_{\mathrm{cons}},\beta_{\mathrm{div}}$.
\Repeat
    ...
    \]
    \State Update $\zeta,\psi$ with $\mathcal{L}_{\mathrm{FM}}+\lambda_{\mathrm{align}}\mathcal{L}_{\mathrm{ReAlign}}$.
...
\begin{table}[!htbp]
    ...
    \begin{tabular}{lcccccccccc}
        \toprule
        Experiment & Predictor & Pair sampling & Time cond. & $\lambda_{\mathrm{align}}$ & $\beta_{\mathrm{cons}}$ & $\rho_{\mathrm{path}}$ & $\beta_{\mathrm{div}}$ & EMA decay & Target detach & Extra cost \\
        \midrule
...
We state the residual-transition identity only to clarify the transition notation used by ReAlign. For a DiT/SiT-style stack~\citep{vaswani2017attention,peebles2023scalable,ma2024sit}, let $A_i^t$ be the token activation entering block $i$ at denoising timestep $t$, and let $c$ denote the conditioning signal. A standard block can be written as two residual branches,
\begin{equation}
\bar A_i^t
=
...
\begin{figure}[!htbp]
    ...
    \end{minipage}}
    \caption{\textbf{Residual view of a DiT stack.} Placeholder schematic for the residual transition used by ReAlign notation.}
...
\begin{table}[!htbp]
    \centering
    \caption{\textbf{HumanML3D setup.} Motion-generation configuration used by the baseline and ReAlign variant.}
...
\begin{table}[!htbp]
    ...
    \begin{tabular*}{0.98\linewidth}{@{\extracolsep{\fill}}p{0.22\linewidth}p{0.29\linewidth}p{0.31\linewidth}p{0.11\linewidth}@{}}
        ...
        \Cref{tab:humanml3d_results} & FID, R-Precision & Use the standard HumanML3D evaluator, split, text encoder, and motion representation for baseline and ReAlign. & \citep{guo2022generating,tevet2022mdm} \\
...
\begin{table}[!htbp]
    ...
    \begin{tabular*}{0.98\linewidth}{@{\extracolsep{\fill}}p{0.24\linewidth}p{0.36\linewidth}p{0.28\linewidth}@{}}
        ...
        Lambda tuning & Set the ReAlign weight without changing the main recipe & FID, prediction error, stability \\
...
\begin{table}[!htbp]
    \centering
    \caption{\textbf{Lambda and schedule controls.} Placeholder for tuning the ReAlign weight, maximum layer gap, and pair/timestep schedule.}
    ...
    \begin{tabular*}{0.96\linewidth}{@{\extracolsep{\fill}}lcccc@{}}
        \toprule
        Setting & $\lambda_{\mathrm{align}}$ & Max gap & Pair / timestep schedule & FID $\downarrow$ \\
        \midrule
